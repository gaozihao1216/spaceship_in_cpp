...
struct TopKSummary {
    int case_count = 0;
    double total_problem2_ms = 0.0;
    int total_missing = 0;
    int total_direct_fallback_skipped = 0;
};
...
spaceship_cpp::problem2::Problem2GravityAssistSolverOptions make_base_problem2_options() {
    ...
    options.topology_max_depth = 10;
    options.max_transfer_revolution = 1;
    options.max_target_revolution = 1;
    return options;
}

spaceship_cpp::problem2::Problem2GravityAssistSolverOptions with_fallback_config(
    spaceship_cpp::problem2::Problem2GravityAssistSolverOptions options,
    ...
    if (config_name == "baseline") {
        options.problem1_fallback_mode = problem1::Problem1FallbackMode::DirectSolve;
        options.problem1_cell_vertex_routeA_enabled = false;
        options.problem1_scout_no_direct_fallback = false;
    } else if (config_name == "hybrid") {
        options.problem1_fallback_mode = problem1::Problem1FallbackMode::CellVertexRouteA;
        options.problem1_cell_vertex_routeA_enabled = true;
        options.problem1_scout_no_direct_fallback = false;
    } else {
        options.problem1_fallback_mode = problem1::Problem1FallbackMode::ScoutNoDirectFallback;
        options.problem1_cell_vertex_routeA_enabled = true;
        options.problem1_scout_no_direct_fallback = true;
    }
    options.problem1_cell_vertex_seed_policy = 1;
    options.problem1_cell_vertex_top_k_vertices = top_k;
    options.problem1_cell_vertex_max_branch_attempts = 8;
    return options;
}

spaceship_cpp::problem2::Problem2GravityAssistSolverResult run_case(
    ...
    const double local_theta = bfs::global_periapsis_angle_to_problem2_local(
        selected.attempt.from_planet, selected.attempt.parent_incoming_theta);
    return spaceship_cpp::problem2::solve_problem2_gravity_assist_with_table(
        loader, selected.attempt.from_planet, selected.attempt.to_planet,
        selected.attempt.parent_time, selected.attempt.parent_incoming_e, local_theta, options);
}
...
void print_case(
    ...
    std::cout << "Problem1CellVertexRouteAScoutCase\n";
    ...
    std::cout << "problem2_total_ms=" << p.total_ms << '\n';
    std::cout << "nearest_query_count=" << p.problem1_nearest_node_query_count << '\n';
    std::cout << "nearest_query_total_ms=" << p.nearest_query_total_ms << '\n';
    std::cout << "fallback_direct_solve_count=" << p.nearest_query_fallback_direct_solve_count << '\n';
    std::cout << "fallback_direct_solve_ms=" << p.nearest_query_fallback_direct_solve_ms << '\n';
    std::cout << "cell_vertex_routeA_attempt_count=" << p.cell_vertex_routeA_attempt_count << '\n';
    std::cout << "cell_vertex_routeA_success_count=" << p.cell_vertex_routeA_success_count << '\n';
    std::cout << "cell_vertex_routeA_failure_count=" << p.cell_vertex_routeA_failure_count << '\n';
    std::cout << "direct_fallback_avoided_count=" << p.direct_fallback_avoided_count << '\n';
    std::cout << "direct_fallback_skipped_count=" << p.direct_fallback_skipped_count << '\n';
    std::cout << "cell_vertex_loaded_vertex_count=" << p.cell_vertex_loaded_vertex_count << '\n';
    std::cout << "cell_vertex_selected_vertex_count=" << p.cell_vertex_selected_vertex_count << '\n';
    std::cout << "cell_vertex_attempted_branch_count=" << p.cell_vertex_attempted_branch_count << '\n';
    std::cout << "cell_vertex_selection_ms=" << p.cell_vertex_selection_ms << '\n';
    std::cout << "cell_vertex_routeA_ms=" << p.cell_vertex_routeA_ms << '\n';
    std::cout << "cell_vertex_validation_ms=" << p.cell_vertex_validation_ms << '\n';
}
...
void print_comparison(
    ...
    const auto c = compare_solutions(selected.attempt.parent_time, baseline.solutions, result.solutions);
    std::cout << "Problem1CellVertexRouteAScoutComparison\n";
    std::cout << "case_name=" << selected.name << '\n';
    std::cout << "config_name=" << config_name << '\n';
    std::cout << "baseline_solution_count=" << baseline.solutions.size() << '\n';
    std::cout << "config_solution_count=" << result.solutions.size() << '\n';
    std::cout << "matched_count=" << c.matched_count << '\n';
    std::cout << "missing_from_config_count=" << c.missing << '\n';
    std::cout << "new_in_config_count=" << c.added << '\n';
    std::cout << "max_abs_arrival_time_diff=" << c.max_arrival_diff << '\n';
    std::cout << "max_abs_alpha_diff=" << c.max_alpha_diff << '\n';
    std::cout << "max_abs_theta_prime_diff=" << c.max_theta_diff << '\n';
    std::cout << "matches_baseline=" << (c.matches ? 1 : 0) << '\n';
}
...
int main() {
    ...
    if (!std::filesystem::exists(table_dir)) {
        std::cout << "problem1_cell_vertex_routeA_scout_fallback_skipped_missing_table\n";
        return 0;
    }
    ...
    const auto base_options = make_base_problem2_options();
    ...
    if (!search.ok) {
        std::cout << "problem1_cell_vertex_routeA_scout_fallback_search_failed\n";
    ...
    const auto slowest_vm = std::max_element(attempts.begin(), attempts.end(), [](const auto& a, const auto& b) {
        const bool av = same_pair(a, spaceship_cpp::planet_params::PlanetId::Venus,
                                  spaceship_cpp::planet_params::PlanetId::Mars);
        const bool bv = same_pair(b, spaceship_cpp::planet_params::PlanetId::Venus,
                                  spaceship_cpp::planet_params::PlanetId::Mars);
        if (av != bv) {
            return !av;
        }
    ...
    if (slowest_vm != attempts.end() &&
        same_pair(*slowest_vm, planet_params::PlanetId::Venus, planet_params::PlanetId::Mars)) {
    ...
    std::map<int, TopKSummary> top_k_summary;
    for (const auto& selected_case : selected) {
        const auto baseline = run_case(loader, selected_case, with_fallback_config(base_options, "baseline", 2));
        print_case(selected_case, "baseline", baseline);

        const auto hybrid = run_case(loader, selected_case, with_fallback_config(base_options, "hybrid", 2));
        print_case(selected_case, "hybrid", hybrid);
        print_comparison(selected_case, "hybrid", baseline, hybrid);

        const auto scout = run_case(loader, selected_case, with_fallback_config(base_options, "scout", 2));
        print_case(selected_case, "scout", scout);
        print_comparison(selected_case, "scout", baseline, scout);

        for (const int top_k : {1, 2, 4}) {
            const auto scout_top_k =
                run_case(loader, selected_case, with_fallback_config(base_options, "scout", top_k));
            const auto comparison =
                compare_solutions(selected_case.attempt.parent_time, baseline.solutions, scout_top_k.solutions);
            auto& summary = top_k_summary[top_k];
            summary.case_count += 1;
            summary.total_problem2_ms += scout_top_k.profile.total_ms;
            summary.total_missing += comparison.missing;
            summary.total_direct_fallback_skipped += scout_top_k.profile.direct_fallback_skipped_count;
        }
    }

    int recommended_top_k = -1;
    double best_score = std::numeric_limits<double>::infinity();
    for (const auto& [top_k, summary] : top_k_summary) {
        const double mean_ms = summary.case_count > 0 ?
            summary.total_problem2_ms / static_cast<double>(summary.case_count) : -1.0;
        const double score = static_cast<double>(summary.total_missing) * 1.0e6 + mean_ms;
        if (score < best_score) {
            best_score = score;
    ...
    for (const auto& [top_k, summary] : top_k_summary) {
        const double mean_ms = summary.case_count > 0 ?
            summary.total_problem2_ms / static_cast<double>(summary.case_count) : -1.0;
        std::cout << "Problem1CellVertexRouteATopKSummary\n";
        std::cout << "top_k=" << top_k << '\n';
        std::cout << "case_count=" << summary.case_count << '\n';
        std::cout << "mean_problem2_total_ms=" << mean_ms << '\n';
        std::cout << "total_missing_from_baseline=" << summary.total_missing << '\n';
        std::cout << "total_direct_fallback_skipped=" << summary.total_direct_fallback_skipped << '\n';
        std::cout << "recommended_top_k=" << recommended_top_k << '\n';
    }

    std::cout << "problem1_cell_vertex_routeA_scout_fallback_ok=1\n";
    return 0;
}
