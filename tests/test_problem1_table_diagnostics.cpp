...
int main() {
    ...
    // 中文说明：构造小规模 Earth→Mars 三轴采样表，作为后续汇总与 CSV 导出测试输入。
    ...
    // 中文说明：验证 summarize_problem1_table 的 cell/branch 计数与有效几何 cell 范围自洽。
    ...
    // 中文说明：当存在有效几何 cell 时，汇总的最/最大 departure 半径与 transfer e 应为有限值。
    ...
    // 中文说明：验证分支诊断的 cell 划分、conic 分布与 (k,q) 圈数范围与表格配置一致。
    ...
    // 中文说明：验证 write_problem1_table_csv 与 branch summary CSV 能写入非空文件到 results/ 目录。
